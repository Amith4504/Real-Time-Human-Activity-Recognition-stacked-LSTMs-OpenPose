"""Classses :
        1. ClassifierOnlineTest
                                -reset
                                -predict
                                -predict_RNN
                                -smooth_scores
                                -draw_scores_into_image
        2.ClassifierOfflineTrain
                                -choose_model
                                -init all models
                                -train
                                -train_RNN
...
# My
from .LSTM_RAR_class import LSTM_classifier
from .feature_proc import FeatureGenerator
from .funcs import int2str
#from .LSTM_RAR import #class
# Sklearn
...
class ClassifierOnlineTest(object):
    ...
    def __init__(self, model_path, action_types, human_id=0):
        ...
        self.THRESHOLD_SCORE_FOR_DISP = 0.4
        self.LSTM_classify = LSTM_classifier()
    ...
    def predict(self, skeleton): 
        ...
        # Generates features and classification
        if is_features_good:
    ...
    def predict_RNN(self,skeleton):  # requires 32x36 input
        LABEL_UNKNOWN = ""
        # CALLED HERE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        curr_scores = self.LSTM_classify.Predict(skeleton)
        self.scores = curr_scores
        #self.scores = self.smooth_scores(curr_scores)
        print(curr_scores)
        ...
        if self.scores.max() < self.THRESHOLD_SCORE_FOR_DISP:
            ...
            predicted_idx = self.scores.argmax()
            predicted_label = self.action_types[predicted_idx]
...
class ClassifierOfflineTrain(object):
    def __init__(self):
        ...
        #self.clf = self.choose_model("LSTM") #changed
    ...
    def train(self, X, Y):
        ...
        #MLPClassifier.fit(X_new, Y)
        self.clf.fit(X_new,Y)
	
    def train_RNN(self): # CHECK IF ARGUEMENTS REQUIRED
        LSTM_classifier.Train()
        # TRAINING DONE , THE ARGUEMENTS ARE PR0VIDED IN THE FUNCTION ITSELF
    ...
    def predict_LSTM(self,skeleton): # required 32x36 input

        Y_pred = LSTM_classifier.Predict(skeleton)  # OFFLINE train predictions for accuracies
        return Y_pred
